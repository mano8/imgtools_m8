...
class TestImageToolsHelper:
    ...
    def test_find_best_combination():
        ...
        with pytest.raises(ImgToolsException):
            ImageToolsHelper.find_best_combination(total=-1, numbers=[2, 3, 4])

        with pytest.raises(ImgToolsException):
            ImageToolsHelper.find_best_combination(total=0, numbers=[])
    ...
    def test_find_all_combinations():
        ...
        with pytest.raises(ImgToolsException):
            ImageToolsHelper.find_all_combinations(total=-1, numbers=[2, 3, 4])

        with pytest.raises(ImgToolsException):
            ImageToolsHelper.find_all_combinations(total=0, numbers=[])
    ...
    def test_get_images_list():
        ...
        files = ImageToolsHelper.get_images_list(HelperTest.get_source_path())
        assert len(files) == 4
    ...
    def test_get_files_list():
        ...
        assert len(files) == 6
        files = ImageToolsHelper.get_files_list(
            sources, ext=".jpg", content_name="bad_"
        )
    ...
    def test_cut_file_name():
        ...
        assert ImageToolsHelper.cut_file_name(file_name="EDSR_x2.pb") == (
            "EDSR_x2",
            ".pb",
        )
        assert ImageToolsHelper.cut_file_name(file_name="img.jpg") == ("img", ".jpg")
        assert ImageToolsHelper.cut_file_name(file_name="img") == ("img", "")
        assert ImageToolsHelper.cut_file_name(file_name="img.tar.gz") == (
            "img.tar",
            ".gz",
        )
        assert ImageToolsHelper.cut_file_name(
            file_name="img.back.tar.gz", ext_len=2
        ) == ("img.back", ".tar.gz")
        assert ImageToolsHelper.cut_file_name(
            file_name="img.back.tar.gz", ext_len=0
        ) == ("img", ".back.tar.gz")
    ...
    def test_get_extension():
        ...
        assert (
            ImageToolsHelper.get_extension(path="img.back.tar.gz", ext_len=2)
            == ".tar.gz"
        )
        assert (
            ImageToolsHelper.get_extension(path="img.tar.gz.sav", ext_len=3)
            == ".tar.gz.sav"
        )
        assert (
            ImageToolsHelper.get_extension(path="img.tAr.gZ.sAv", ext_len=3)
