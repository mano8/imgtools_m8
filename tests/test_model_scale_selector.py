...
class TestModelScaleSelector:
    ...
    def test_get_model_scale_needed():
        ...
        with pytest.raises(ImgToolsException):
            ModelScaleSelector.get_model_scale_needed(width=0, height=22)

        with pytest.raises(ImgToolsException):
            ModelScaleSelector.get_model_scale_needed(width=22, height=0)
    ...
    def test_count_upscale():
        ...
        with pytest.raises(ImgToolsException):
            ModelScaleSelector.count_upscale(width=0, height=22, model_scale=2)

        with pytest.raises(ImgToolsException):
            ModelScaleSelector.count_upscale(width=22, height=0, model_scale=2)

        with pytest.raises(ImgToolsException):
            ModelScaleSelector.count_upscale(width=22, height=22, model_scale=0)
    ...
    def test_get_scale_stats():
        ...
        args = [{"x_scales": [4, 50], "combination": [2 for x in range(2, 50, 2)]}]
        for params in args:
            with pytest.raises(ImgToolsException):
    ...
    def test_scale_combination_analytics():
        ...
        args = [
            ...
            },
            {
                ...
                "possibilities": ModelScaleSelector.get_best_scale_combinations(
                    max_x_scale=14, available_scales=[2, 3, 4]
                ),
            },
        ]
        results = []
        ...
        args = [
            {
                ...
                "scale_analytics": [[0, 2, 3], [0, 1, 1], [0, 2, 5], [0, 0, 0]],
            },
            {
                "stats": [
                    {"key": 3, "x_scale": 0},
    ...
    def test_define_model_scale():
        ...
        stats = ModelScaleSelector.get_max_upscale(
            size=size, output_formats=output_formats
        )
        ModelScaleSelector.define_model_scale(
            upscale_stats=stats, available_scales=[2, 3, 4]
        )
    ...
    def test_get_upscale_stats():
        ...
        stats = ModelScaleSelector.get_upscale_stats(
            size=size, output_formats=output_formats, model_scale=2
        )
