...
class TestImageUtils:
    ...
    def test_is_valid_image():
        ...
        assert ImageUtils.is_valid_image(
            join(source_path, 'cat1', 'mar.jpg'))
        assert not ImageUtils.is_valid_image(
            join(source_path, 'cat1', 'mar.exe'))
        assert not ImageUtils.is_valid_image(
            join(source_path, 'cat1', 'bad_file.txt'))

    @staticmethod
    def test_get_image_size():
        """Test get_image_size method"""
        source_path = HelperTest.get_source_path()
        size = ImageUtils.get_image_size(
            join(source_path, 'cat1', 'mar.jpg'))
        assert size == (276, 397)
        size = ImageUtils.get_image_size(
            join(source_path, 'cat1', 'mar.exe'))
    ...
    def test_get_image_format():
        ...
        source_path = HelperTest.get_source_path()
        image_format = ImageUtils.get_image_format(
            join(source_path, 'cat1', 'mar.jpg'))
        assert image_format == "JPEG"
        image_format = ImageUtils.get_image_size(
            join(source_path, 'cat1', 'mar.exe'))
    ...
    def test_get_image_format_type(size, result):
        ...
        image_format = ImageUtils.get_image_format_type(
            image_size=size)
    ...
    def test_is_valid_size(size, result):
        ...
        image_format = ImageUtils.is_valid_size(
            size=size)
        assert image_format == result
    ...
    @staticmethod
    def test_invalid_get_new_scale(
        size,
        fixed_width,
        fixed_height
    ):
