...
class TestScanDir:
    ...
    def test_get_path_directory_list():
        ...
        result, nb_sub_dirs = ScanDir.get_path_directory_list(
            source_path=source_path, root=join(source_path, "cat1")
        )
        ...
        result, nb_sub_dirs = ScanDir.get_path_directory_list(
            source_path=source_path, root=join(source_path, "bad_dir")
        )
        assert result is None
        assert nb_sub_dirs == 0

        result, nb_sub_dirs = ScanDir.get_path_directory_list(
            source_path=source_path, root=source_path + sep
        )
        assert result is None
        assert nb_sub_dirs == 0
    ...
    @pytest.mark.parametrize(
        ...
        [
            (join(HelperTest.get_source_path(), "bad_dir"), "mar.jpg", None),
            (HelperTest.get_source_path(), "mar.exe", None),
            (HelperTest.get_source_path(), ".private_file", None),
            (HelperTest.get_source_path(), "a.txt", None),
            (HelperTest.get_source_path(), "mar.jpg", "mar.jpg"),
        ],
    )
    @staticmethod
    def test_get_file_item(root, img_file, expected):
        """Test get_file_item method"""
        result = ScanDir.get_file_item(root=root, file=img_file)
    ...
    def test_scan_tree():
        ...
        for root, file, dirs, nb_sub_dirs in ScanDir.scan_tree(
            source_path=source_path, max_depth=4
        ):
            result.append(
                {"root": root, "file": file, "dirs": dirs, "nb_sub_dirs": nb_sub_dirs}
            )
        assert isinstance(result, list)
    ...
    def test_restricted_scan_tree():
        ...
        result = []
        for root, file, dirs, nb_sub_dirs in ScanDir.scan_tree(
            source_path=source_path, max_depth=1
        ):
            result.append(
                {"root": root, "file": file, "dirs": dirs, "nb_sub_dirs": nb_sub_dirs}
            )
    ...
    def test_get_ordered_files():
        ...
        result = ScanDir.get_ordered_files(
            source_path=HelperTest.get_source_path(), order_by="image_format"
        )
        assert isinstance(result, dict)
        ...
        result = ScanDir.get_ordered_files(
            source_path=HelperTest.get_source_path(), order_by="image_size"
        )
        assert isinstance(result, dict)
        assert isinstance(result.get("portrait"), dict)
