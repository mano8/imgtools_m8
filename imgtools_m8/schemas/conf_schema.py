...
class JpegFormat(BaseModel):
    ...
    ext: Literal[OutputFormatsEnum.JPEG] = Field(
        OutputFormatsEnum.JPEG, description="JPEG format."
    )
    quality: Optional[int] = Field(
        None, ge=1, le=100, description="JPEG quality (1-100)."
    )
    ...
    subsampling: Optional[Union[int, Literal["4:4:4", "4:2:2", "4:2:0"]]] = Field(
        None, description="Chroma subsampling: integer or string format."
    )
    ...
    def check_constraints(self) -> "JpegFormat":
        ...
        if self.subsampling is not None and not self.is_subsampling_valid(
            self.subsampling
        ):
            raise ValueError(
...
class WebpFormat(BaseModel):
    ...
    ext: Literal[OutputFormatsEnum.WEBP] = Field(
        OutputFormatsEnum.WEBP, description="WEBP format."
    )
    quality: Optional[int] = Field(
        None, ge=1, le=100, description="WebP quality (1-100)."
    )
    lossless: Optional[bool] = Field(False, description="Enable lossless compression.")
...
class PngFormat(BaseModel):
    ...
    ext: Literal[OutputFormatsEnum.PNG] = Field(
        OutputFormatsEnum.PNG, description="PNG format."
    )
    optimize: Optional[bool] = Field(False, description="Enable PNG optimization.")
    compression_level: Optional[int] = Field(
        None, ge=0, le=9, description="Compression level (0-9)."
    )
    interlace: Optional[bool] = Field(False, description="Enable PNG interlacing.")
...
class GifFormat(BaseModel):
    ...
    ext: Literal[OutputFormatsEnum.GIF] = Field(
        OutputFormatsEnum.GIF, description="GIF format."
    )
    optimize: Optional[bool] = Field(False, description="Enable GIF optimization.")
...
class AvifFormat(BaseModel):
    ...
    ext: Literal[OutputFormatsEnum.AVIF] = Field(
        OutputFormatsEnum.AVIF, description="AVIF format."
    )
    quality: Optional[int] = Field(
...
class OutputSize(BaseModel):
    ...
    fixed_size: Optional[int] = Field(
        None,
        gt=0,
        description="Resize preserving aspect ratio, largest dimension constrained.",
    )
...
class OutputOptions(BaseModel):
    ...
    def check_constraints(self) -> "OutputOptions":
        ...
        if self.image_size is not None and self.image_size.fixed_upscale is not None:
        ...
        if not (
            ...
            raise ValueError(
                "At least one of image_size, max_byte_size, or formats must be set."
            )
        return self
...
class GlobalOutputOptions(BaseModel):
    ...
    def check_constraints(self) -> "GlobalOutputOptions":
        ...
        if not (self.max_byte_size is not None or self.formats is not None):
            raise ValueError(
                "At least one of image_size, max_byte_size, or formats must be set."
            )
        return self
...
class ImageProcessingSchema(BaseModel):
    ...
    source_path: str = Field(..., description="Path to image file or directory.")
    include_subdirs: Optional[bool] = Field(
        False, description="Process subdirectories (up to depth 4)."
    )
    output_path: str = Field(..., description="Output directory for saved images.")
    flatten_output: Optional[bool] = Field(
        False, description="Flatten all outputs into one dir if using subdirs."
    )
